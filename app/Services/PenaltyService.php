...
class PenaltyService
{
    ...
    /**
 * Calculate penalties - ATOMIC (UPDATED to prevent duplicates)
 */
    ...
    return DB::transaction(function () use ($agreementType, $agreementId) {
        try {
            ...
            if ($overdueSchedules->isEmpty()) {
                Log::info("No overdue schedules - clearing any orphaned pending penalties");
                
                // Only delete penalties that have no corresponding overdue schedule
                Penalty::where('agreement_type', $agreementType)
                    ->where('agreement_id', $agreementId)
                    ->where('status', 'pending')
                    ->whereNotIn('payment_schedule_id', 
                ...
                return ['penalties_created' => 0, 'penalties_updated' => 0, 'message' => 'No overdue schedules'];
            }
        ...
        for ($month = 1; $month <= $monthsOverdue; $month++) {
            ...
            if ($existingPenalty) {
                ...
                if ($existingPenalty->status === 'pending') {
                    ...
                    $penaltiesUpdated++;
                    Log::info("Updated existing penalty", ['penalty_id' => $existingPenalty->id]);
                } else {
                    Log::info("Skipping non-pending penalty", [
                        'penalty_id' => $existingPenalty->id,
                        'status' => $existingPenalty->status
                    ]);
                }
            } else {
                // ⭐ CREATE NEW PENALTY
                Penalty::create(array_merge($penaltyData, [
                    'agreement_type' => $agreementType,
                    'agreement_id' => $agreementId,
                ...
                $penaltiesCreated++;
                Log::info("Created new penalty for month {$month}");
            }
        }
    }

    Log::info("Final Payment calculation complete", [
...
private function calculateContinuingInstallmentPenalties($agreementType, $agreementId, $overdueSchedules)
{
    ...
    $cumulativeUnpaidInstallments = 0;
    ...
    foreach ($overdueSchedules as $index => $schedule) {
        ...
        $existingPenalty = Penalty::where('agreement_type', $agreementType)
            ->where('agreement_id', $agreementId)
            ->where('payment_schedule_id', $schedule->id)
            ->where('penalty_sequence', $index + 1)
            ->first();
        ...
        if ($existingPenalty) {
            ...
            $penaltiesCreated++;
            Log::info("Created new penalty for schedule {$schedule->installment_number}");
        }
